---
title: "Unity URP로 구현한 종말지 캐릭터 Toon Rendering 정리"
...
`【unity urp】从零模仿复刻实现自己的终末地人物卡通渲染` 글을, 원문 흐름을 최대한 유지하면서 자연스럽게 옮겨 정리한다.

작성자는 이전 카드렌더링 글 이후 한동안 다른 연구 때문에 글을 쉬었다가, 이번에는 `终末地` 스타일의 캐릭터 rendering을 `Unity URP`에서 직접 복각한 과정을 정리한다. 목표는 단순 아이디어 소개가 아니라, 처음부터 따라가면 비슷한 결과를 직접 만들 수 있을 정도로 전체 구현을 설명하는 것이다.
...
글은 구현 전에 반드시 확보해야 하는 리소스부터 정리한다. `toon base shader`에는 기본적으로 `albedo`, `normal`, material property texture가 필요하다. 이 property map에는 `metallic`, `reflectivity`, `ao`, `smoothness` 같은 값이 들어 있고, `albedo`의 alpha 채널은 cutout이나 semi-transparent material 구현에 연결된다.
...
이 글에서 가장 중요한 포인트는 `终末地` 스타일을 “PBR 위에 toon trick을 얹은 구조”로 해석한다는 점이다. 완전히 flat한 anime 조명으로 가지 않고, 물리적인 light distribution은 어느 정도 유지하되, 그 위에 artist control을 강하게 추가한다.

핵심 설계는 대략 이렇다.

- 직사광 상태를 별도 파라미터로 관리한다
- main light 외에 캐릭터 전용 top light를 둔다
- diffuse는 물리 기반 분포 + three-tone color layering + ramp 보정으로 만든다
- specular와 rim light는 camera-aware한 stylization을 넣는다
- skin, face, transparent, metal은 base shader에서 파생한다

즉, 이 글은 단순 shader 코드 소개보다도, “하나의 anime character rendering system을 어떻게 계층화해 설계할 것인가”에 더 가깝다. `Unity URP`에서 비슷한 방향을 잡으려면 좋은 참조다.
