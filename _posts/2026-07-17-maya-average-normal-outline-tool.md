...
2026-07-17 기준으로 Zhihu 원문 링크는 다시 확인했지만, 현재 작업 환경에서는 여전히 본문 직접 접근이 되지 않았다. 검색 스니펫과, 원문을 참조한 GitHub 구현 정보를 기준으로 정리한다. 그래서 아래 내용은 `원문 전문 직역`이 아니라 `확인 가능한 내용 중심 정리`다.
