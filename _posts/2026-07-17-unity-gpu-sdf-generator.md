---
title: "Unity에서 GPU로 돌리는 SDF 생성기 정리"
...
`【技术美术】unity 工具记录 - 写个GPU上运行的SDF生成器` 글을, 원문 흐름을 최대한 유지하면서 자연스럽게 옮겨 정리한다.
...
이 글이 좋은 이유는 `SDF`를 추상적인 수학 도구로 설명하는 대신, 실제 기술미술 workflow 안에 바로 연결한다는 점이다. Houdini에서 만든 연소나 성장 애니메이션을 그대로 엔진으로 가져오면, 보통은 frame별 mask를 재생하는 수준에서 멈추기 쉽다. 하지만 `SDF`로 바꾸면 threshold, edge width, gradient 누적 같은 후처리를 엔진에서 훨씬 유연하게 제어할 수 있다.

또 알고리즘 선택 기준도 실무적이다. 절대적으로 가장 빠른 이론보다, `GPU 병렬 실행에 맞는가`를 우선으로 보고 `Saito`를 고른다는 판단이 분명하다. 그래서 이 글은 단순 번역 거리보다도, Unity 기술미술 툴을 직접 짤 때 어떤 관점으로 문제를 푸는지 보는 데 가치가 있다.
