...
Colorful Palette의 글 `プロジェクトセカイ 3周年グラフィックスアップデート解説 (目の透過表現)`을, 가능한 한 원문 흐름을 유지하면서 자연스럽게 옮겨 정리한다.
...
글 후반부에서는 실제 shader 속성 구성과 C# 세팅 구조도 보여준다.

- 일반 pass용 stencil 파라미터와 투과 pass용 stencil 파라미터를 분리
- shader property만으로 각 캐릭터와 재질의 동작을 바꿀 수 있게 설계
- C# 쪽에서는 재질 타입에 맞는 stencil 설정값을 만들어 주는 factory 구조 사용

즉, 이건 단순 shader trick이 아니라:
...
- 원문: <https://media.colorfulpalette.co.jp/n/n927a776b6b35>
  프로세카 3주년 그래픽스 업데이트 중 눈 투과 표현 구현을 설명한 글이다.
